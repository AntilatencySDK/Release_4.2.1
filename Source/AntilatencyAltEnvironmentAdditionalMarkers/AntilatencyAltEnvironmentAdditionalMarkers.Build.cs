...
namespace UnrealBuildTool.Rules {
    public class AntilatencyAltEnvironmentAdditionalMarkers : ModuleRules {

        public AntilatencyAltEnvironmentAdditionalMarkers(ReadOnlyTargetRules Target) : base(Target) {
...
#endif
                var environmentAdditionalMarkersLibraryPath = Path.Combine(binaryDirectory, "AntilatencyAltEnvironmentAdditionalMarkers.dll");

#if UE_4_19_OR_LATER
                RuntimeDependencies.Add(environmentAdditionalMarkersLibraryPath);
#else
                RuntimeDependencies.Add(new RuntimeDependency(environmentAdditionalMarkersLibraryPath));
#endif
            } else if (Target.Platform == UnrealTargetPlatform.Android) {
                var pluginPath = Utils.MakePathRelativeTo(ModuleDirectory, Target.RelativeEnginePath);
                var aplPath = Path.Combine(pluginPath, "AntilatencyAltEnvironmentAdditionalMarkers_APL.xml");
#if UE_4_19_OR_LATER
