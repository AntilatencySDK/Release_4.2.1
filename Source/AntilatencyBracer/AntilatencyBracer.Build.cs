...
namespace UnrealBuildTool.Rules {
	public class AntilatencyBracer : ModuleRules {
		public AntilatencyBracer(ReadOnlyTargetRules Target) : base(Target) {
...
#endif
                var bracerLibraryPath = Path.Combine(binaryDirectory, "AntilatencyBracer.dll");

#if UE_4_19_OR_LATER
                RuntimeDependencies.Add(bracerLibraryPath);
#else
                RuntimeDependencies.Add(new RuntimeDependency(bracerLibraryPath));
#endif
            } else if (Target.Platform == UnrealTargetPlatform.Android){
	            var pluginPath = Utils.MakePathRelativeTo(ModuleDirectory, Target.RelativeEnginePath);
                var aplPath = Path.Combine(pluginPath, "AntilatencyBracer_APL.xml");
#if UE_4_19_OR_LATER
