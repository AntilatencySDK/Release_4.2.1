...
namespace UnrealBuildTool.Rules {
    public class AntilatencyHardwareExtensionInterface : ModuleRules {

        public AntilatencyHardwareExtensionInterface(ReadOnlyTargetRules Target) : base(Target) {
...
#endif
                var hardwareExtensionInterfaceLibraryPath = Path.Combine(binaryDirectory, "AntilatencyHardwareExtensionInterface.dll");

#if UE_4_19_OR_LATER
				RuntimeDependencies.Add(hardwareExtensionInterfaceLibraryPath);
#else
                RuntimeDependencies.Add(new RuntimeDependency(hardwareExtensionInterfaceLibraryPath));
#endif
            } else if (Target.Platform == UnrealTargetPlatform.Android) {
                var pluginPath = Utils.MakePathRelativeTo(ModuleDirectory, Target.RelativeEnginePath);
                var aplPath = Path.Combine(pluginPath, "AntilatencyHardwareExtensionInterface_APL.xml");
#if UE_4_19_OR_LATER
